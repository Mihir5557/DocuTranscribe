...
class OCRExtractor:
    def __init__(self, det_arch, reco_arch, pretrained, detect_language):
        self.model = ocr_predictor(det_arch, reco_arch, pretrained=pretrained, detect_language=detect_language)
    ...
    def extract(self, file_path, show_prediction=False):
        ...
        for data_file in tqdm(sorted((f for f in os.listdir(data_path) if not f.startswith(".")), key=str.lower)):
            ...
            with open(ocr_path + data_file.replace('.jpg', '').replace('.png', '').replace('.jpeg', '') + '.json', 'w') as f:
